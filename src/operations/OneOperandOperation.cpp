...
#include "OneOperandOperation.h"
#include <iostream>
#include <iomanip>

OneOperandOperation::OneOperandOperation(Processor* processor) : BaseOperation(processor) {

}

void OneOperandOperation::decode() {
	uint16_t instruction = processor->getCurrentInstruction();
    uint16_t mod = (instruction & 070) >> 3;
    uint16_t reg = instruction & 007;
	uint16_t isByteOp = instruction &0100000;
	if (isByteOp) {
		address = processor->getModAddressByte(mod, reg);
	} else {
